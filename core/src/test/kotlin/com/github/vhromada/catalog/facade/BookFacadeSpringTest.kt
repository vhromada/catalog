...
class BookFacadeSpringTest {
    ...
    fun searchPaging() {
        ...
        assertSoftly {
            ...
            it.assertThat(result.pagesCount).isEqualTo(BookUtils.BOOKS_COUNT)
        }
    ...
    fun add() {
        ...
        BookUtils.assertBookDeepEquals(expected = expectedDomainBook, actual = BookUtils.getDomainBook(entityManager = entityManager, id = BookUtils.BOOKS_COUNT + 1))
    ...
    fun duplicate() {
        ...
        val expectedDomainBook = BookUtils.getDomainBook(index = 1)
            .copy(id = BookUtils.BOOKS_COUNT + 1, uuid = TestConstants.UUID)
            .fillAudit(audit = AuditUtils.newAudit())
        val expectedDomainBookItems = expectedDomainBook.items.mapIndexed { index, bookItem ->
            bookItem.copy(id = BookItemUtils.BOOK_ITEMS_COUNT + index + 1, uuid = TestConstants.UUID, book = expectedDomainBook)
                .fillAudit(audit = AuditUtils.newAudit())
        }
