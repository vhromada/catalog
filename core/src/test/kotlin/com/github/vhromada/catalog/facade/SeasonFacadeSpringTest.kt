...
class SeasonFacadeSpringTest {
    ...
    fun findAllPaging() {
        for (i in 1..ShowUtils.SHOWS_COUNT) {
            ...
            assertSoftly {
                ...
                it.assertThat(result.pagesCount).isEqualTo(SeasonUtils.SEASONS_PER_SHOW_COUNT)
            }
    ...
    fun add() {
        ...
        SeasonUtils.assertSeasonDeepEquals(expected = expectedDomainSeason, actual = SeasonUtils.getDomainSeason(entityManager = entityManager, id = SeasonUtils.SEASONS_COUNT + 1))
    ...
    fun duplicate() {
        ...
        val expectedDomainSeason = show.seasons.first()
            .copy(id = SeasonUtils.SEASONS_COUNT + 1, uuid = TestConstants.UUID)
            .fillAudit(audit = AuditUtils.newAudit())
        show.seasons.add(expectedDomainSeason)
        val expectedDomainEpisodes = expectedDomainSeason.episodes.mapIndexed { index, episode ->
            episode.copy(id = EpisodeUtils.EPISODES_COUNT + index + 1, uuid = TestConstants.UUID, season = expectedDomainSeason)
                .fillAudit(audit = AuditUtils.newAudit())
        }
