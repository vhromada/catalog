...
class ShowFacadeSpringTest {
    ...
    fun search() {
        ...
        assertSoftly {
            it.assertThat(result.pageNumber).isEqualTo(1)
            it.assertThat(result.pagesCount).isEqualTo(1)
        }
    ...
    fun searchPaging() {
        ...
        assertSoftly {
            it.assertThat(result.pageNumber).isEqualTo(2)
            it.assertThat(result.pagesCount).isEqualTo(ShowUtils.SHOWS_COUNT)
        }
    ...
    fun add() {
        ...
        ShowUtils.assertShowDeepEquals(expected = expectedDomainShow, actual = ShowUtils.getDomainShow(entityManager = entityManager, id = ShowUtils.SHOWS_COUNT + 1))
    ...
    fun duplicate() {
        val expectedShow = ShowUtils.getShow(index = 1)
            .copy(uuid = TestConstants.UUID)
        val expectedDomainShow = ShowUtils.getDomainShow(index = 1)
            .copy(id = ShowUtils.SHOWS_COUNT + 1, uuid = TestConstants.UUID)
            .fillAudit(audit = AuditUtils.newAudit())
        val expectedDomainSeasons = expectedDomainShow.seasons.mapIndexed { index, season ->
            val expectedDomainSeason = season.copy(id = SeasonUtils.SEASONS_COUNT + index + 1, uuid = TestConstants.UUID, show = expectedDomainShow)
                .fillAudit(audit = AuditUtils.newAudit())
            val expectedDomainEpisodes = expectedDomainSeason.episodes.mapIndexed { episodeIndex, episode ->
                episode.copy(id = EpisodeUtils.EPISODES_COUNT + EpisodeUtils.EPISODES_PER_SEASON_COUNT * index + episodeIndex + 1, uuid = TestConstants.UUID, season = expectedDomainSeason)
                    .fillAudit(audit = AuditUtils.newAudit())
            }
