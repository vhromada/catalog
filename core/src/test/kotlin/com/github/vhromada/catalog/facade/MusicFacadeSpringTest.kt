...
class MusicFacadeSpringTest {
    ...
    fun searchPaging() {
        ...
        assertSoftly {
            ...
            it.assertThat(result.pagesCount).isEqualTo(MusicUtils.MUSIC_COUNT)
        }
    ...
    fun add() {
        ...
        MusicUtils.assertMusicDeepEquals(expected = expectedDomainMusic, actual = MusicUtils.getDomainMusic(entityManager = entityManager, id = MusicUtils.MUSIC_COUNT + 1))
    ...
    fun duplicate() {
        ...
        val expectedDomainMusic = MusicUtils.getDomainMusic(index = 1)
            .copy(id = MusicUtils.MUSIC_COUNT + 1, uuid = TestConstants.UUID)
            .fillAudit(audit = AuditUtils.newAudit())
        val expectedDomainSongs = expectedDomainMusic.songs.mapIndexed { index, song ->
            song.copy(id = SongUtils.SONGS_COUNT + index + 1, uuid = TestConstants.UUID, music = expectedDomainMusic)
                .fillAudit(audit = AuditUtils.newAudit())
        }
