...
class MovieValidatorImpl : MovieValidator {
    ...
    override fun validateRequest(request: ChangeMovieRequest) {
        ...
        if (request.imdbCode != null && request.imdbCode != -1 && (request.imdbCode < 1 || request.imdbCode > Constants.MAX_IMDB_CODE)) {
            result.addEvent(event = Event(severity = Severity.ERROR, key = "MOVIE_IMDB_CODE_NOT_VALID", message = "IMDB code must be between 1 and 999999999 or -1."))
        }
